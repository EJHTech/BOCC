...
rm www.ed.ac.uk/profile/*student 2>> log.txt
for alpha in {a..z}; do
  sed -i -e '/Course secretary/,+2d' www.drps.ed.ac.uk/$year/dpt/cx$alpha*.htm 2>> log.txt
done
...
for alpha in {a..z}; do
cp -n ../../www.drps.ed.ac.uk/$year/dpt/cx[$alpha]*.htm ../../www.drps.ed.ac.uk/$year/bak 2>> log.txt
done

for alpha in {a..z}; do
gawk -i inplace -v INPLACE_SUFFIX=.bak '/Reading List/{flag=1;next}/table/{flag=0;next} flag' ../../www.drps.ed.ac.uk/$year/dpt/cx[$alpha]*.htm 2>> log.txt
done

#add full path to filename for join matching and create readingList file--------
for alpha in {a..z}; do
awk '{print FILENAME,$0}' ../../www.drps.ed.ac.uk/$year/dpt/cx[$alpha]*.htm > readingList.txt 2>> log.txt
done
...
#clean up folder
echo "Cleaning up..."
rm -R ../../www.drps.ed.ac.uk/$year/dpt/ 2>> log.txt
cp -R ../../www.drps.ed.ac.uk/$year/bak/ ../../www.drps.ed.ac.uk/$year/dpt/ 2>> log.txt
echo "Done"
